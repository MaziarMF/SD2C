...
def next_batch_masked(num, data, data_sec, data_masked):
    indices = np.arange(0, data.shape[0])
    np.random.shuffle(indices)
    indices = indices[:num]
    batch_data = np.asarray([data[i, :] for i in indices])
    indices_masked = np.arange(0, data_masked.shape[0])
    np.random.shuffle(indices_masked)
    indices_masked = indices_masked[:num]
    masked_data = np.asarray([data_masked[i, :] for i in indices_masked])
    sec_data = np.asarray([data_sec[i, :] for i in indices_masked])
    return indices, batch_data, sec_data, masked_data
...
def write_list(file_name, array):
    ...
    with open(file_name, 'w') as f:
        for item in array:
          f.write("{}\n".format(item))
def stem_tokenize(stemmer, tokenizer, doc):
    # Inspired from https://stackoverflow.com/questions/36182502/add-stemming-support-to-countvectorizer-sklearn
